    ...
    def __init__(self, encryption_key: Optional[str] = None):
        if encryption_key:
            ...
            if not key:
                ...
                print(
                    "Warning: Using generated encryption key. Set SECRETS_ENCRYPTION_KEY env var in production"
                )
...
class SecretsManager:
    ...
    def decrypt_value(self, encrypted_value: str) -> Any:
        ...
        try:
            ...
            if "invalid token" in error_msg or "decrypt" in error_msg:
                print(f"Warning: Failed to decrypt value - likely encrypted with different key. Error: {e}")
                raise ValueError(f"Invalid encryption token - data may have been encrypted with a different key. Original error: {e}")
            else:
                print(f"Warning: Failed to decrypt value due to unexpected error: {e}")
                raise ValueError(f"Decryption failed: {e}")

    def encrypt_config_credentials(
        self, config: Dict[str, Any], credential_keys: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def decrypt_config_credentials(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if decryption_errors:
            print(f"Decryption warnings: {'; '.join(decryption_errors)}")

        return decrypted_config

    async def store_credentials(
        self, data_source_id: str, credentials: Dict[str, Any]
    ) -> str:
        """
    ...
    async def update_credentials(
        self, secret_uri: str, credentials: Dict[str, Any]
    ) -> str:
        """
